...
# In Operator를 이용해 기존 고객으로 조건을 걸고, 추가로 2011년도 매출 건만 조회할 수 있도록 추가 조건 생성하기
# 고객별로 첫 구매 월 계산
SELECT CUSTOMERID,
...
# 첫 구매 월로 그룹핑한 뒤 CUSROMERID 집계
SELECT MM,
       COUNT(CUSTOMERID) N_CUSTOMERS
  FROM (SELECT CUSTOMERID, 
               SUBSTR(INVOICEDATE, 1, 7) MM
	  FROM (SELECT *
                  FROM MYDATA.DATASET3
...
# LTV: 고객과의 미래 관계를 고려해 귀속되는 순이익을 예측하는 지표 

# Retention Rate는 2010년 구매자 중 2011년에 구매한 고객의 비율로 계산
# Retention Rate = t1 and t0 구매 고개 수/t0 시점 구매 고객 수
