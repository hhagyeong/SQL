...
# 상품별 재구매율을 계산하고, 가장 높은 순서대로 순위매기기
SELECT *
  FROM (SELECT *,
	       ROW_NUMBER() OVER(ORDER BY RET_RATIO DESC) RNK
	  FROM (SELECT PRODUCT_ID,
...
# PRODUCT_REPURCHASE_QUANTILE 테이블 생성
...
# 테이블 생성 확인
SELECT *
  FROM INSTACART.PRODUCT_REPURCHASE_QUANTILE;
  
/* 상품별 분위 수: PRODUCT_REPURCHASE_QUANTILE
   주문 소요 시간: INSRACART.ORDERS
   주문 번호와 상품 번호: INSTACART.ORDER_PRODUCTS__PRIOR */
   
# PRODUCT_ID의 DAYS_SINCE_PRIOR_ORDER구하기
SELECT PRODUCT_ID,
...
# 테이블 생성 확인
SELECT *
  FROM INSTACART.ORDER_PRODUCTS__PRIOR2;
  
# 결합한 테이블에서 분위수, 상품별 구매 소요 시간의 분산 계산
SELECT A.RNK_GRP,
       A.PRODUCT_ID,
...
# 각 분위 수의 상품 소요 기간 분산의 증위 수 계산하기
# MySQL에서는 Median 함수를 제공하지 않으므로 avg()함수 이용
SELECT RNK_GRP,
 ...
 GROUP BY 1
 ORDER BY RNK_GRP+1;
 # 결과를 살펴보면, 분위 수에 따라 재구매 주기의 분산에 차이가 없는 것으로 확인됨
 # 즉 이 결과를 통해 재구매율이 높은 상품이라고 해서 구매 주기가 평균에 집중되지 않음을 확인할 수 있음
